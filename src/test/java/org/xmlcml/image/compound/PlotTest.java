...
public class PlotTest {
	...
	public void testPlot() throws IOException {
		BufferedImage image = UtilImageIO.loadImage(new File(G002_DIR,
				"g002.png").toString());
		...
		PixelIslandList islands = PixelIslandList
				.createPixelIslandList(new File(G002_DIR, "g002.png"));
		Assert.assertEquals("plot islands", 91, islands.size());

		PixelIslandList axes = PixelIslandList.createPixelIslandList(new File(
				G002_DIR, "axes.png"));
		Assert.assertEquals("axes", 1, axes.size());

		PixelIslandList errorbar = PixelIslandList
				.createPixelIslandList(new File(G002_DIR, "errorbar.png"));
		Assert.assertEquals("errorbar", 1, errorbar.size());
		PixelIslandList xvalues = PixelIslandList
				.createPixelIslandList(new File(G002_DIR, "xnumbers.png"));
		Assert.assertEquals("xvalues", 11, xvalues.size());
		PixelIslandList xtitle = PixelIslandList
				.createPixelIslandList(new File(G002_DIR, "xtitle.png"));
		Assert.assertEquals("xtitle", 33, xtitle.size());
		PixelIslandList yvalues = PixelIslandList
				.createPixelIslandList(new File(G002_DIR, "ynumbers.png"));
		Assert.assertEquals("yvalues", 9, yvalues.size());
		PixelIslandList ytitle = PixelIslandList
				.createPixelIslandList(new File(G002_DIR, "ytitle.png"));
		Assert.assertEquals("ytitle", 33, ytitle.size());
	}

	@Test
	public void testAxes0() throws IOException {
		PixelIsland axes = PixelIslandList.createPixelIslandList(
				new File(G002_DIR, "axes.png")).get(0);
	...
	public void testErrorBar() throws IOException {
		PixelIsland errorbar = PixelIslandList.createPixelIslandList(
				new File(G002_DIR, "errorbar.png"), Operation.BINARIZE).get(0);
		...
		PixelIslandList thinned = PixelIslandList
				.thinFillAndGetPixelIslandList(
						ImageIO.read(new File(G002_DIR, "errorbar.png")),
	...
	public void testAxes1() throws IOException {
		PixelIsland axes = PixelIslandList.createPixelIslandList(
				new File(G002_DIR, "axes.png"), Operation.BINARIZE).get(0);
		Assert.assertEquals("pixels", 7860, axes.size());
		SVGG gg = new SVGG();
		RingList ringList = axes.createRingsAndPlot(gg, new String[] {
				"orange", "green" });
	...
	public void testXTitle() throws IOException {
		PixelIslandList titleChars = PixelIslandList.createPixelIslandList(
				new File(G002_DIR, "xtitle.png"), Operation.BINARIZE);
		Assert.assertEquals("characters", 33, titleChars.size());
		List<RingList> characterList = titleChars.createRingListList(new File(
				"target/plot/xtitle.svg"));
		SVGG gg = new SVGG();
		PixelIslandList thinned = PixelIslandList
				.thinFillAndGetPixelIslandList(
						ImageIO.read(new File(G002_DIR, "xtitle.png")),
						new ZhangSuenThinning());
	...
	public void testXNumbers() throws IOException {
		PixelIslandList xNumbers = PixelIslandList.createPixelIslandList(
				new File(G002_DIR, "xnumbers.png"), Operation.BINARIZE);
	...
	public void testYTitle() throws IOException {
		PixelIslandList yTitle = PixelIslandList.createPixelIslandList(
				new File(G002_DIR, "ytitle.png"), Operation.BINARIZE);
	...
	public void testYNumbers() throws IOException {
		PixelIslandList yNumbers = PixelIslandList.createPixelIslandList(
				new File(G002_DIR, "ynumbers.png"), Operation.BINARIZE);
	...
	public void testPoints() throws IOException {
		PixelIslandList points = PixelIslandList.createPixelIslandList(
				new File(G002_DIR, "points.png"), Operation.BINARIZE);
		points.createRingListList(new File("target/plot/points.svg"));
		Assert.assertEquals("points", 4, points.size());
		PixelIslandList thinned = PixelIslandList
				.thinFillAndGetPixelIslandList(
						ImageIO.read(new File(G002_DIR, "points.png")),
						new ZhangSuenThinning());
	...
	public void testNodes() throws IOException {
		...
		PixelIslandList thinned = PixelIslandList
				.thinFillAndGetPixelIslandList(
						ImageIO.read(new File(G002_DIR, "points.png")),
						new ZhangSuenThinning());
		thinned.createRingListList(new File("target/plot/points3.svg"));
	}
